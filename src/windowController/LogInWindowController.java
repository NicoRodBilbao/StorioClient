...
public class LogInWindowController {
    ...
    public void logIn(ActionEvent event) {
        try {
            //En caso de que no se valide el campo de usuario con más de 30 caracteres o que haya espacios en blanco, llama al UserManagerException
            ...
            if (tfPassword.getText().length() > 30 || tfPassword.getText().contains(" ")) {
                ...
                throw new UserManagerException("Wrong password");
            } else {
                if(!UserFactory.getAccessUser().loginUser(tfUsername.getText(), tfPassword.getText())) {
                    decorPassword.setBackground(new Background(new BackgroundFill(Color.RED, CornerRadii.EMPTY, Insets.EMPTY)));
                    throw new UserManagerException("Wrong password");
                } else {
                    primaryStage.close();
